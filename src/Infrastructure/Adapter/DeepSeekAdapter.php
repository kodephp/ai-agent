 ...
 * DeepSeek 平台适配器
 * 
 * 实现 DeepSeek API 的统一接口封装，兼容 OpenAI 格式。
 * 使用 kode/http-client 进行 HTTP 请求。
 * 
 * @package Kode\AiAgent\Infrastructure\Adapter
 */
final class DeepSeekAdapter implements AdapterInterface
{
    use StreamHelper;
    ...
    private function sendRequest(PromptInterface $prompt, array $options): array
    {
        ...
        if ($statusCode === 401) {
            throw AuthenticationException::invalidApiKey('deepseek');
        }
    ...
    private function validateConfig(): void
    {
        ...
        if (strlen($this->config['api_key']) < 16) {
            throw ConfigurationException::invalid('api_key', '无效的 DeepSeek API Key 格式');
        }
    }
